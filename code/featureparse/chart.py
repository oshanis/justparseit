...
#
# $Id: chart.py 5782 2008-02-26 05:28:06Z purplebob $

"""
An overridden version of NLTK's L{Chart} class.

This class removes some assumptions that NLTK makes about grammars that prevent
extensibility, such as that every grammar will contain all the words it may
need to parse as string literals within it. The alternative is to allow an
external function to assign tags to words.
"""
...
class TreeEdge(EdgeI):
    ...
    # Comparisons & hashing
    def __cmp__(self, other):
        if self.__class__ != other.__class__: raise ValueError
...
class PredictorRule(CachedTopDownExpandRule):
    def apply_iter(self, chart, grammar, edge):
        ...
        for prod in grammar.productions(lhs=edge.next()):
            ...
            if chart.insert(new_edge, ()):
                yield new_edge
            if new_edge.span() == (0,0) and new_edge.is_complete():
                for index in range(1, chart.num_leaves()+1):
                    newer_edge = TreeEdge((index,index), new_edge.lhs(),
                    new_edge.rhs(), new_edge.dot())
                    if chart.insert(newer_edge, ()):
                        yield newer_edge
